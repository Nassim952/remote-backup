...
class Mailer
{
    ...
    public function sendForgetPwd($userEmail, $userName, $token)
    {
        $mail = new PHPMailer();  // Creer un nouvel objet PHPMailer
        // $mail->IsSMTP(); // active SMTP
        $mail->SMTPDebug = 0;  // debogage: 1 = Erreurs et messages, 2 = messages seulement
        ...
        $mail->Port = 587;
        $mail->SetFrom('admin@gmail.com', 'Nearby Administrateur');
        $mail->Subject = 'Reinitialisation mot de passse';
        $mail->Priority = 1;
        $mail->IsHTML(true);
        $body = '<!DOCTYPE html>
        <html lang="fr">
        ...
        <body>
        <div class="wrapper">
            <p>Cliquer sur le lien ci-dessous pour reinitialiser votre mot de passe !</p>
            <a href="http://localhost:8081/reset-form-pwd/'.$token.'">Reset mot de passe</a>
        </div>
        </body>
    ...
    public function sendConfirmReservation($userEmail, $movie, $nbrTicket, $dateSeance, $cinema)
    {
        $mail = new PHPMailer();  // Creer un nouvel objet PHPMailer
        // $mail->IsSMTP(); // active SMTP
        $mail->SMTPDebug = 0;  // debogage: 1 = Erreurs et messages, 2 = messages seulement
        ...
        <body>
        <div class='wrapper'>
            <p> Votre reservation pour le film $movie le $dateSeance au cinema $cinema est bien confirmee !</p>
            <br/>
            <p> Vous avez commander $nbrTicket tickets. <p>
        </div>
        </body>
